namespace PommaLabs.KVLite.Utilities.Text
{
    ...
    {
        private readonly System.Collections.Generic.Dictionary<string, System.Collections.Generic.List<TokenOccurrence>> OccurrencesOfToken;
        ...
        public FastReplacer(string tokenOpen, string tokenClose, bool caseSensitive = true)
        {
            ...
            {
                throw new System.ArgumentException("Token must have opening and closing delimiters, such as \"{\" and \"}\".");
            }

            _tokenOpen = tokenOpen;
            _tokenClose = tokenClose;

            var stringComparer = caseSensitive ? System.StringComparer.Ordinal : System.StringComparer.OrdinalIgnoreCase;
            OccurrencesOfToken = new System.Collections.Generic.Dictionary<string, System.Collections.Generic.List<TokenOccurrence>>(stringComparer);
        }
        ...
        public FastReplacer Replace(string token, string text, out bool result)
        {
            ...
            System.Collections.Generic.List<TokenOccurrence> occurrences;
        ...
        private void ExtractTokens(FastReplacerSnippet snippet)
        {
            ...
            while (last < snippet.Text.Length)
            {
                ...
                var start = snippet.Text.IndexOf(_tokenOpen, last, System.StringComparison.OrdinalIgnoreCase);
                if (start == -1)
                {
                    return;
                }
                var end = snippet.Text.IndexOf(_tokenClose, start + _tokenOpen.Length, System.StringComparison.OrdinalIgnoreCase);
                if (end == -1)
                {
                    throw new System.ArgumentException(string.Format("Token is opened but not closed in text \"{0}\".", snippet.Text));
                }
                ...
                {
                    OccurrencesOfToken.Add(token, new System.Collections.Generic.List<TokenOccurrence> { tokenOccurrence });
                }
        ...
        private void ValidateToken(string token, string context, bool alreadyValidatedStartAndEnd)
        {
            ...
            {
                if (!token.StartsWith(_tokenOpen, System.StringComparison.OrdinalIgnoreCase))
                {
                    throw new System.ArgumentException(string.Format("Token \"{0}\" shoud start with \"{1}\". Used with text \"{2}\".", token, _tokenOpen, context));
                }
                var closePosition = token.IndexOf(_tokenClose, System.StringComparison.OrdinalIgnoreCase);
                if (closePosition == -1)
                {
                    throw new System.ArgumentException(string.Format("Token \"{0}\" should end with \"{1}\". Used with text \"{2}\".", token, _tokenClose, context));
                }
                if (closePosition != token.Length - _tokenClose.Length)
                {
                    throw new System.ArgumentException(string.Format("Token \"{0}\" is closed before the end of the token. Used with text \"{1}\".", token, context));
                }
            }

            if (token.Length == _tokenOpen.Length + _tokenClose.Length)
            {
                throw new System.ArgumentException(string.Format("Token has no body. Used with text \"{0}\".", context));
            }
            if (token.Contains("\n"))
            {
                throw new System.ArgumentException(string.Format("Unexpected end-of-line within a token. Used with text \"{0}\".", context));
            }
            if (token.IndexOf(_tokenOpen, _tokenOpen.Length, System.StringComparison.OrdinalIgnoreCase) != -1)
            {
                throw new System.ArgumentException(string.Format("Next token is opened before a previous token was closed in token \"{0}\". Used with text \"{1}\".", token, context));
            }
        }
        ...
        public override string ToString()
        {
            ...
            var sb = new System.Text.StringBuilder(totalTextLength);
    ...
    internal sealed class FastReplacerSnippet
    {
        ...
        private readonly System.Collections.Generic.List<InnerSnippet> _innerSnippets;

        public FastReplacerSnippet(string text)
        {
            Text = text;
            _innerSnippets = new System.Collections.Generic.List<InnerSnippet>();
        }
